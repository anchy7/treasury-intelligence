...
# `In Hubspot` and `Last Contacted` are computed from crm_all_companies.csv
...
# ADDED: Suggested Topics + Last Projects (enrichment placeholders)
# RENAMED: Posted Date -> Job Run Date, In CRM -> In Hubspot, Letzter Kontakt -> Last Contacted
...
# without the leading ï»¿ BOM sneaking into the first cell header. German
# characters (ö, ä, ü, ß) still render correctly in Excel as long as Excel
# opens the file via Data → From Text/CSV (not by double-click, which on some
# locales defaults to the system code page).
CSV_ENCODING = "utf-8"
...
def assign_responsible(row) -> str:
    ...
    - Germany AND (Kyriba OR Nomentia OR "liquidity management"/"liquiditätsmanagement" in title) → Sven
    - Germany AND (ION OR "debt"/"schulden"/"verbindlichkeiten" OR "finanzierung" in title) → Stephan
    - SAP technology → Alexander
    - Germany AND ("cash management" / "zahlungsverkehr" in title) → Stefan
    - Switzerland AND ("risk management" / "risikomanagement" in title) → Elia
    - Everything else in Switzerland → Tobias
    - Everything else in Germany → Carsten

    German equivalents of the English keywords are also matched so postings
    written in German fall into the right bucket.
    """
    ...
    # Keyword bundles (English + German equivalents)
    has_liquidity_mgmt = ("liquidity management" in title
                          or "liquiditätsmanagement" in title
                          or "liquiditaetsmanagement" in title
                          or "liquiditätssteuerung" in title)
    has_debt = ("debt" in title
                or "finanzierung" in title
                or "schulden" in title
                or "verbindlichkeiten" in title
                or "fremdkapital" in title)
    has_cash_mgmt = ("cash management" in title
                     or "cash-management" in title
                     or "zahlungsverkehr" in title)
    has_risk_mgmt = ("risk management" in title
                     or "risikomanagement" in title
                     or "risiko management" in title)

    # 1) Germany + Kyriba / Nomentia / liquidity management → Sven
    if is_germany and ("kyriba" in tech or "nomentia" in tech or has_liquidity_mgmt):
        return "Sven"

    # 2) Germany + ION / debt / finanzierung → Stephan
    if is_germany and ("ion" in tech or has_debt):
        return "Stephan"

    # 3) SAP technology (any country) → Alexander
    if "sap" in tech:
        return "Alexander"

    # 4) Germany + cash management → Stefan
    if is_germany and has_cash_mgmt:
        return "Stefan"

    # 5) Switzerland + risk management → Elia
    if is_switzerland and has_risk_mgmt:
        return "Elia"

    # 6) Everything else in Switzerland → Tobias
    if is_switzerland:
        return "Tobias"

    # 7) Everything else (incl. Germany default) → Carsten
    return "Carsten"
# ---------------------------------------------------------------------------
...
def load_crm(path: Path) -> pd.DataFrame:
    ...
    try:
        # CRM file has its own encoding (utf-8-sig) because it often comes out
        # of Excel with a BOM — read it tolerantly regardless of the main
        # export's CSV_ENCODING.
...
def save_to_local_onedrive(df: pd.DataFrame, filename: str = "treasury_jobs_export.csv") -> bool:
    ...
    try:
        ...
        # Save the CSV (same separator + encoding as the primary export)
...
def load_jobs(path: Path) -> pd.DataFrame:
    ...
    # OUTPUT written by this script uses CSV_SEPARATOR. We read with
    # encoding='utf-8-sig' so a BOM on the input, if present, is transparently
    # stripped.
...
def _refresh_crm_cols_in_existing(existing: pd.DataFrame, crm: pd.DataFrame) -> pd.DataFrame:
    """
    CRM fields (`In Hubspot`, `Last Contacted`) are derived — not a historical
...
def merge_with_existing(new_df: pd.DataFrame, out_path: Path, crm: pd.DataFrame | None = None) -> pd.DataFrame:
    ...
    if not out_path.exists():
        ...
        return new_df
    try:
        # Read back the previous run's output using the same separator it was
        # written with. `utf-8-sig` here is deliberate (not CSV_ENCODING) — it
        # transparently handles both plain UTF-8 (current format) and legacy
...
def main() -> int:
    ...
    p.add_argument("--crm", default=DEFAULT_CRM_FILE,
                   help=f"CRM CSV used to compute 'In Hubspot' + 'Last Contacted' "
